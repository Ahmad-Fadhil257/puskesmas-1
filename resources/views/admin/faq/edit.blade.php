...
@section('content')
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-3">
        <div>
            <h4 class="fw-bold py-1 mb-1" style="color: #0A5C45;">
                <i class="bx bx-edit-alt me-2"></i>Ubah Pertanyaan FAQ
            </h4>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-header border-bottom py-3">
            <h5 class="card-title mb-0 fw-bold">Ubah Informasi Pertanyaan &amp; Jawaban</h5>
        </div>
            ...
            <form action="{{ route('admin.faq.update', $faq->id) }}" method="POST">
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6 col-12">
                        ...
                        <select class="form-select @error('kategori') is-invalid @enderror" id="kategori" name="kategori" required>
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat }}" {{ old('kategori', $faq->kategori) === $cat ? 'selected' : '' }}>{{ $cat }}</option>
                            @endforeach
                        </select>
                        <div class="form-text small">Pilih salah satu topik kategori untuk memudahkan navigasi masyarakat.</div>
                        @error('kategori')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <div class="form-text small">Gunakan kalimat yang jelas dan cantumkan persyaratan berkas jika ada.</div>
                        @error('jawaban')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
